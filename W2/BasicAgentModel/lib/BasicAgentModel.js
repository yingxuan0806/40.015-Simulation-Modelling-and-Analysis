...
// set loccation of doctor and reception on the screen
// so that these variables become targets for the patients' simulation to reach the doctor and receptionist
var doctorRow = 10;
var doctorCol = 20;
...
// first things first, declare variables, define constants for the model
...
// here, the different states of the entire sequence. total of 7 states for the patient
const UNTREATED = 0;
const WAITING = 1;
const STAGING = 2; 
const INTREATMENT = 3;
...
// total of 2 states for the doctor
...
// set up the locations of the fixed agents. only patients move
...
// set up locations of the waiting areas on the screen
var areas = [
	{
		"label": "Waiting Area",
		"startRow": 4,
...
function updateSurface() {
	...
	// ie. initially, there is no elements of class patient, so we use the entering list of data elements to
	// create the matching list of svg elements
	var newpatients = allpatients.enter().append("g").attr("class","patient"); 
	// append an image for each new patient:
	...
	newpatients.append("svg:image")
	 .attr("x", function(d) {var cell = getLocationCell(d.location); return cell.x + "px";}) // anonymous function
	...
	// patient's location is updated by the simulation every animationDelay milliseconds
	// make it appear as a continuous movement using transition() function (ie. not jumping cell to cell)
	images.transition()
	...
	// initially, there are no elements of class caregiver, hence we use the entering list of data elements to
	// create the matching list of svg elements
	...
	// Append/Create a svg group ("g") for each new entry in the data list; give it class "caregiver"
	// ie. here, we append an invisible svg element
	...
	// here we specify the type of svg element (to add the display to the interface)
	// image will be a child element of the group element
	...
	// the statistics are updated at each time step
	...
	// ie. display rectangles for areas
	...
	// we use the attributes of the data element to control the location and appearance of each rectangle
...
function addDynamicAgents(){
	...
	if (Math.random() < probArrival) { //every sim step we add at most one patient, according to the probArrival probability
		var newpatient = { // for each new patient we create an object with attributes
			"id": 1,
			"type": "A",
			"location":
				{
		...
		// assign type A or B patient according to probaility probTypeA
		if (Math.random() < probTypeA) newpatient.type = "A";
		else newpatient.type = "B";			
		patients.push(newpatient); // add this object to patients array
		// ie. new patients are added at the end of the array
	}
...
function updatePatient(patientIndex){
	...
	// use nested switch statements to handle all different cases
	switch(state){
		case UNTREATED:
			if (hasArrived){
...
function removeDynamicAgents(){
	// in every sim step, we need to remove patients who have been discharged. 
	// cannot use .exit().remove() function combination, because this assumes that the elements to be removed
	// are at the end of the array
	// current situation: exiting patients can be scattered throughout the array. not neccessarily at the end
	// hence this removeDynamicAgents() function is created for this purpose. 


	// Select all svg elements of class "patient" and map it to the data list called patients
	var allpatients = surface.selectAll(".patient").data(patients);
	// Select all the svg groups of class "patient" whose state is EXITED
	// use filter function to find the svg elements that match the data elements that are about to be deleted
	var treatedpatients = allpatients.filter(function(d) {return d.state == EXITED;});
	// an example of functional programming (same idea as lambda function)
	// Remove the svg groups of EXITED patients: they will disappear from the screen at this point
	// ie. exiting patients are removed using a filter. 
	// note: exiting patients can be scattered throughout the array. not necessarily only at the end.
	treatedpatients.remove(); // delete svg elements (not the array elements)
	// if we remove the data element first, there would not be a way for us to know which svg element to remove on screen.
	
	// Remove the EXITED patients from the patients list using a filter command
	// after deleting the svg elements, delete the array elements
	patients = patients.filter(function(d) {return d.state != EXITED;}); // lambda function
	// lambda function: must return true/false value -> true values are returned
	...
	// this is an example of functional programming: remove patients who have exited (instead of using a for loop to remove)
}


// every sim step we loop over each dynamic agent (ie. patient)
// update its state and location
// call updateSurface to animate the display
// all the interesting modelling takes place in the function updatePatient()
function updateDynamicAgents() {
	// loop over all the agents and update their states
	for (var patientIndex in patients) {
		updatePatient(patientIndex);
	}
	updateSurface(); //for graphics and animation interface
}


// simulation loop
// seta a timer to call this function repeatedly
function simStep() {
